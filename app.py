            ...
            if not df.empty:
                ...
                while True:
                    for value in unique_scrutiny_values:
                        ...
                        prioritized_rows = df[df['scrutiny_status'] == value].sample(frac=1).reset_index(drop=True)

                        # Shuffle remaining rows that do not have the current `scrutiny_status` value
                        remaining_rows = df[df['scrutiny_status'] != value].sample(frac=1).reset_index(drop=True)
                        ...
                        st.write(f"Currently prioritizing entries for `scrutiny_status`: **{value}**")
                        time.sleep(30)  # Interval for each filter display
